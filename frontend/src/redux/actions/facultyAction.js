...
export const facultyLogin = (credentials) => {
  return async (dispatch) => {
    try {
      const { data } = await axios.post("http://localhost:5000/api/faculty/login", credentials);
      const { token } = data;
  ...
  return async (dispatch) => {
    try {
      const { data } = await axios.post(
        "http://localhost:5000/api/faculty/updatePassword",
        passwordData
      );
...
export const getOTPFaculty = (email) => {
  return async (dispatch) => {
    try {
      await axios.post("http://localhost:5000/api/faculty/forgotPassword", email);
...
export const submitOTPFaculty = (credentials) => {
  return async (dispatch) => {
    try {
      const { data } = await axios.post("http://localhost:5000/api/faculty/postOTP", credentials);
...
export const fetchStudents = (department, year, section) => {
  return async (dispatch) => {
    try {
      const { data } = await axios.post("http://localhost:5000/api/faculty/fetchStudents", {
        department,
        year,
        section,
      });
  ...
  return async (dispatch) => {
    try {
      const { data } = await axios.put(
        "http://localhost:5000/api/faculty/updateProfile",
        updatedData
      );
...
export const markAttendance = (
  ...
  return async (dispatch) => {
    try {
      await axios.post("http://localhost:5000/api/faculty/markAttendance", {
        selectedStudents,
        subjectCode,
        department,
        year,
...
export const uploadMarks = (
  ...
  return async (dispatch) => {
    try {
      await axios.post("http://localhost:5000/api/faculty/uploadMarks", {
        subjectCode,
        exam,
        totalMarks,
        marks,
